...
import { getDelegates } from 'apiSdk/delegates';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['president', 'delegate'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<DelegateInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/delegates` : null),
    () => getDelegates({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Delegate',
      path: isTenantUser ? `/delegates/view/${data?.[0]?.id}` : '/delegates',
      entity: 'delegate',
